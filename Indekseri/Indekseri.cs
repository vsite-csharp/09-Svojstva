    ...
    class PoštanskiBrojevi
    {
        ...
        public string this[int poštanskiBroj]
        {
            get
            {
                ...
                {
                    if (poštanskiBroj == grad.Key)
                    {
                        return grad.Value;
                    }
                }
                throw new ArgumentException("Poštanski broj nije sadržan u popisu");
            }
        }

        // 042 Definirati svojstvo sa string kao parametrom koje će za zadano mjesto vratiti poštanski broj.
        public int this[string imeGrada]
        {
            get
            {
                foreach (var grad in popis)
                {
                    if (imeGrada == grad.Value)
                    {
                        return grad.Key;
                    }
                }
                throw new ArgumentException("Naziv grada nije sadržan u popisu");
            }
        }
    }
