...
namespace Vsite.CSharp.Svojstva
{
    ...
    class PoštanskiBrojevi
    {
        ...
        public string this[int poštanskiBroj]
        {
            get
            {
                bool isMjesto = popis.TryGetValue(poštanskiBroj, out string mjesto);
                if (!isMjesto)
                    throw new ArgumentException($"Mjesto s poštanskim brojem {poštanskiBroj} ne postoji.");
                return mjesto;
            }
        }
        ...
        public int this[string mjesto]
        {
            get
            {
                foreach(var element in popis)
                {
                    if (element.Value == mjesto)
                        return element.Key;
                }
                throw new ArithmeticException($"Mjesto {mjesto} nema definiran poštanski broj.");
            }
        }
    }
