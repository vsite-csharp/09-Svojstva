...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            {
                get => ime;
                //set => ime = (value ?? throw new ArgumentNullException($"{ nameof(value) }Ime must not be null reference")).Length > 0 ? value : throw new ArgumentException($"{ nameof(value) }Ime must not be empty");
                set
                {
                    //ime = (value ?? throw new ArgumentNullException($"{ nameof(value) }Ime must not be null reference")).Length > 0 ? value : throw new ArgumentException($"{ nameof(value) }Ime must not be empty");
                    if (value == null)
                        throw new ArgumentNullException($"{ nameof(value) }Ime must not be null reference");
                    if (value.Length == 0)
                        throw new ArgumentException($"{ nameof(value) }Ime must not be empty");
                    ime = value;
                }
            }
