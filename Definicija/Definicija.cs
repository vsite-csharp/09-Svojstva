...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            {
                get => ime; 
               // set =>ime= ime = (value ?? throw new ArgumentNullException($"{nameof(value)}Ime must not be null reference")).Length > 0 ? value : throw new ArgumentNullException($"{nameof(value)}Ime must not be null reference");
               set
                {
                   // ime=(value ??throw new ArgumentNullException($"{nameof(value)}Ime must not be null reference")).Length>0 ?value : throw new ArgumentNullException($"{nameof(value)}Ime must not be null reference");
                    if (value == null)
