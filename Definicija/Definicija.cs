...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            {
                get => ime;
                set => ime = (value ?? throw new ArgumentNullException($"{nameof(value)} must not be null reference!")).Length > 0 ? value : throw new ArgumentNullException($"{nameof(value)} must not be an empty string!");
            }
            ...
            public DateTime DatumRođenja
            {
                ...
                set
                {
                    ...
                    {
                        throw new ArgumentOutOfRangeException($"{nameof(value)} must not be greater than todays date");
                    }
                    datumRođenja = value;
                }
            }
