    ...
    class Definicija
    {
        public class Osoba
        {
            ...
            public string Ime
            {
                ...
                get => ime;
                set {
                    //ime=(value?? throw new ArgumentException($"{ nameof(value) }Imen must not be null reference");)
                    if (value == null)
                        throw new ArgumentNullException($"{ nameof(value) }Imen must not be null reference");
                    if (value.Length == 0)
