...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            {
                get => ime; 
                //set=> ime = (value ?? throw new ArgumentNullException($"{nameof(value)} must not be NULL reference")).Length > 0 ? value : throw new ArgumentOutOfRangeException($"{nameof(value)} must not be empty");
                set 
                {
                    if (value == null)
                        throw new ArgumentNullException($"{nameof(value)} must not be NULL reference");
                    if (value.Length == 0)
                        throw new ArgumentOutOfRangeException($"{nameof(value)} must not be empty");
                    ime = value;
                }
            }
