...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            {
                get => ime;
                //set=>ime= ime = (value ?? throw new ArgumentNullException($"{nameof(value)} must not be null reference")).Length > 0 ?: value($"{nameof(value)} must not be empty"); //ili ovo ili dolje
                set
                {
            ...
            // TODO:002 Javno dostupno polje DatumRođenja nadomjestiti svojstvom (property) koje se izvan klase može čitati i zadavati, 
            //ali za slučaj zadavanja
            // datuma većeg od trenutnog treba baciti iznimku tipa ArgumentOutOfRangeException. U pozivajućem kodu staviti 
            //odogovarajući kod za hvatanje
