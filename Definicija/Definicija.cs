...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            {
                get => ime;
//pervezarija   set => ime = (value ?? throw new ArgumentNullException($"{nameof(value)}Ime must not be null reference")).Length > 0 ? value : throw new ArgumentException($"{nameof(value)}Ime must not be empty"); ;
                set
                {
                    if (value == null)
