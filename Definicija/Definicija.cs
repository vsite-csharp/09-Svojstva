...
namespace Vsite.CSharp.Svojstva
{
    class Definicija
    {
        public class Osoba
        {
            ...
            public string Ime {
                get { return ime; }
                //set => ime =(value?? throw new ArgumentNullException($"{nameof(value)}Ime must not be null reference")).Length>0?value : throw new ArgumentException($"{nameof(value)}Ime bust not be empty");
            set
                {
                    ...
                    if (value.Length == 0)
                    {
                        throw new ArgumentException($"{nameof(value)}Ime bust not be empty");
                    }
                    ime = value;
                }
            }
            ...
            public DateTime DatumRođenja
            {
                ...
                set
                {
                    ...
                    {
                        throw new ArgumentOutOfRangeException($"{nameof(value)} mast ");
                    }
                }
            }
        }
